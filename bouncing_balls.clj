...
(defn bouncing-balls-func [h bounce window]
  (if (and (> h 0) (> bounce 0) (< bounce 1) (< window h))
    (loop [hbounce  (* h bounce)
           quantity 1]
      (if (<= hbounce window)
        quantity
        (recur (* hbounce bounce) (+ quantity 2))))
    -1)


  )

(bouncing-balls-func 3 0.66 1.5);; => 3
(bouncing-balls-func 3 1 1.5);; => -1
